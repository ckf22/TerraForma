...
myvec2 find_closest_voronoi(index_t x, index_t y, value_t cell_size, value_t cell_count, std::vector<std::vector<myvec2>>& points, long temp_i){
    // Cell cordinates
    ...
    for(long x1 = std::max(0L, x_idx-2); x1 < x_idx+2 && x1 < points.size(); ++x1){
        for(long y1 = std::max(0L, y_idx-2); y1 < y_idx+2 && y1 < points[x1].size(); ++y1){
    ...
    //for(long x1 = 0; x1 < points.size(); ++x1){
    //    for(long y1 = 0; y1 < points[x1].size(); ++y1){
    //        value_t distance_buffer = glm::distance(points[x1][y1], {c_xd,c_yd});
    //        if(distance_buffer < closest_distance){
    //            closest_distance = distance_buffer;
    //            ret = points[x1][y1];
    //        }
    ...
    if( !ret.has_value() ){
        throw std::logic_error(std::to_string(temp_i)+"; scale: "+std::to_string(cell_size)+"; HeightMap cords: ("+std::to_string(x)
...
HeightMap apply_octave(HeightMap& terrain, value_t erosion_scale, value_t erosion_strength){
    ...
    HeightMap new_terrain{terrain.get_dimension()-1};
    ...
    for(index_t x = 0; x < terrain.get_dimension()-1; ++x){
        for(index_t y = 0; y < terrain.get_dimension()-1; ++y){
            auto cords = find_closest_voronoi(x,y, erosion_scale, voronoi_dimension, voronoi_points, temp_i++);
            // determine slope vector
            myvec2 gradient{
                terrain[{x+1,y}] - terrain[{x,y}],
                terrain[{x,y+1}] - terrain[{x,y}]
            };
            if(temp_i > 1000 && temp_i % 50000 == 340){
                std::cout << "pivot point: (" << cords.x << '|' << cords.y << ") heightmap: (" << x << '|' << y << "); Gradient: (" << gradient.x << '|' << gradient.y << ");" << std::endl;
                //std::cout << 
            }
            // Input into stripe function}
            new_terrain[{x,y}] = get_stripe_value({x,y}, {.pivot_point = cords, .rotation_vector = gradient, .frequency = .08, .amplitude = 1});
        }
    }
